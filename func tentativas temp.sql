...
-- funcs: atividade_users *, favoritos_biblioteca *, autor_avaliacao, IDs inusados, recomendar_livro, adicionar emprestimo/reserva **
-- nº funcs: 7 (- 4) feitas: 5
-- POR ACABAR: favoritos_biblioteca

DROP FUNCTION atividade_livros_user();
...
BEGIN
RETURN QUERY
	SELECT T1.id_aluno as ids, T1.n_emprestimos, T2.n_acessos, T2.n_reservas
	...
	NATURAL JOIN ( select ace.id_aluno,ace.n_acessos,res.n_reservas 
					...
					NATURAL JOIN ( select T5.id_aluno,COUNT(reserva_id) as n_reservas FROM ( select id as id_aluno FROM utilizador) as T5
						...
						GROUP BY T5.id_aluno) as res) as T2;
END;
$pro$
LANGUAGE plpgsql;
...
-- fim ----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS autor_avaliacao(autor VARCHAR);
...
DROP FUNCTION IF EXISTS ids_inusados(entidade TEXT) CASCADE;
...
DROP FUNCTION IF EXISTS recomendar_livro(INT);
...
-- VIEWS---------------------------------------------------------
-- quantidadade de dados, livros_disp_emprestar, utilizadores e multas
